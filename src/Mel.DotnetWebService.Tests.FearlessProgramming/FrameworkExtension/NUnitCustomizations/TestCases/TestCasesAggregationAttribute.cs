...
public sealed class TestCasesAggregationAttribute : NUnitAttribute, ITestBuilder, IImplyFixture
{
	...
	static readonly Dictionary<Type, MethodInfo> TestCasesAggregationsAggregationInstanciatorsByPropertyType = [];
	...
	static object GetSourceValue(PropertyInfo property, Type testCaseType)
	{
		...
		var isAggregationOfAggregations = property.PropertyType.IsOrImplementsGenericInterface(typeof(IEnumerable<>), out var argTypes) && argTypes[0].IsGenericType && argTypes[0].GetGenericTypeDefinition() == typeof(TestCasesAggregation<>);
		if (isAggregationOfAggregations)
		{
			...
			{
				testCasesAggregationInstanciator = typeof(TestCasesAggregation<>)
					.MakeGenericType(testCaseType)
					.GetMethod(
						nameof(TestCasesAggregation<object>.CreateFromTestCasesAggregations),
						BindingFlags.Public | BindingFlags.Static);

				TestCasesAggregationInstanciatorsByPropertyType.Add(property.PropertyType, testCasesAggregationInstanciator);
			}

			return testCasesAggregationInstanciator.Invoke(null, [value]);
		}
		else
		{
			if (!TestCasesAggregationsAggregationInstanciatorsByPropertyType.TryGetValue(property.PropertyType, out var testCasesAggregationInstanciator))
			{
				testCasesAggregationInstanciator = typeof(TestCasesAggregation<>)
					.MakeGenericType(testCaseType)
					.GetMethod(
				...
				TestCasesAggregationsAggregationInstanciatorsByPropertyType.Add(property.PropertyType, testCasesAggregationInstanciator);
			}

			return testCasesAggregationInstanciator.Invoke(null, [value, property.Name]); 
		}
