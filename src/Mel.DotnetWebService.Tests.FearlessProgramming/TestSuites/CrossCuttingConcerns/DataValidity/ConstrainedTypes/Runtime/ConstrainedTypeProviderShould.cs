...
class ConstrainedTypeProviderShould
{
	...
	public void ProvideConcreteExampleValuesBothValidAndInvalid(Type type)
	{
		...
		foreach (var validValue in constrainedType.ValidValueExamples)
		{
			...
			Assert.DoesNotThrow(action, $"{type.GetName()}.{nameof(constrainedType.InvokeStaticFactoryMethod)}({validValue.GetStringRepresentation()}) should have been valid but was not.");
		}
		...
		foreach (var constraintViolationExample in constrainedType.ErrorMessagesByInvalidNativeRootTypeValueExample)
		{
			...
			Assert.That(
				...
				$"{type.GetName()}.{nameof(constrainedType.InvokeStaticFactoryMethod)}({constraintViolationExample.ExampleValue.GetStringRepresentation()}) did not trigger the expected invalidation.");
		}
	}
