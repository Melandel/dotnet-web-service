...
class SerializationShould
{
	static JsonSerializerOptions OptionsInvolvingConstrainedTypesHandling
	{
		get
		{
			...
			v.Converters.Add(new KeyValuePairsWithComplexKeyTypeJsonConverter());
	...
	//		// Act
	//		var deserialized = JsonSerializer.Deserialize(serialized, deserializationType, OptionsInvolvingConstrainedTypesHandling);
	//		var reserialized = JsonSerializer.Serialize(deserialized,                      OptionsInvolvingConstrainedTypesHandling);
	...
	[TestCase(typeof(FirstClassCollectionOfArraysType))]
	[TestCase(typeof(FirstClassCollectionOfArraysConstructedFromIEnumerableType))]
	[TestCase(typeof(FirstClassCollectionOfArraysConstructedFromListType))]
	[TestCase(typeof(FirstClassCollectionOfListsType))]
	[TestCase(typeof(FirstClassCollectionOfListsConstructedFromIEnumerableType))]
	[TestCase(typeof(FirstClassCollectionOfListsConstructedFromListType))]
	[TestCase(typeof(FirstClassCollectionOfIReadOnlyCollectionsType))]
	[TestCase(typeof(FirstClassCollectionOfIReadOnlyCollectionsConstructedFromIEnumerableType))]
	[TestCase(typeof(FirstClassCollectionOfIReadOnlyCollectionsConstructedFromListType))]
	public void Foo(Type dataType)
	{
		//var dataType = typeof(FirstClassCollectionOfArraysType);
		...
		var serialized = "[[[[\"00000000-0000-0000-0000-000000000001\"]],[[\"00000000-0000-0000-0000-000000000001\"]]],[[[\"00000000-0000-0000-0000-000000000001\"]],[[\"00000000-0000-0000-0000-000000000001\"]]]]";
		foreach (var deserializationType in new[] { listOfTArrays, listOfTLists, listOfTIEnumerables, listOfTNonEmptyArrays })
		{
			var deserialized = JsonSerializer.Deserialize(serialized, deserializationType, OptionsInvolvingConstrainedTypesHandling);
			var reserialized = JsonSerializer.Serialize(deserialized, OptionsInvolvingConstrainedTypesHandling);
			Assert.That(reserialized, Is.EqualTo(serialized));
		}
		foreach (var deserializationType in new[] { arrayOfTArrays, arrayOfTLists, arrayOfTIEnumerables, arrayOfTNonEmptyArrays })
		{
			var deserialized = JsonSerializer.Deserialize(serialized, deserializationType, OptionsInvolvingConstrainedTypesHandling);
			var reserialized = JsonSerializer.Serialize(deserialized, OptionsInvolvingConstrainedTypesHandling);
			Assert.That(reserialized, Is.EqualTo(serialized));
		}
		foreach (var deserializationType in new[] { iEnumerableOfTArrays, iEnumerableOfTLists, iEnumerableOfTIEnumerables, iEnumerableOfTNonEmptyArrays })
		{
			var deserialized = JsonSerializer.Deserialize(serialized, deserializationType, OptionsInvolvingConstrainedTypesHandling);
			var reserialized = JsonSerializer.Serialize(deserialized, OptionsInvolvingConstrainedTypesHandling);
			Assert.That(reserialized, Is.EqualTo(serialized));
		}
		foreach (var deserializationType in new[] { nonEmptyArrayOfTArrays, nonEmptyArrayOfTLists, nonEmptyArrayOfTIEnumerables, nonEmptyArrayOfTNonEmptyArrays })
		{
			var deserialized = JsonSerializer.Deserialize(serialized, deserializationType, OptionsInvolvingConstrainedTypesHandling);
			var reserialized = JsonSerializer.Serialize(deserialized, OptionsInvolvingConstrainedTypesHandling);
			Assert.That(reserialized, Is.EqualTo(serialized));
	...
	[Test]
	//[TestCaseSource(typeof(SerializationTestCases.That_Involve_Constrained_Types))]
	public void Render_Data_That_InvolvesConstrainedTypes_AsIfTheyWereRootType()
	//public void Render_Data_That_InvolvesConstrainedTypes_AsIfTheyWereRootType(string serialized, Type deserializationType)
	{
		var errors = new Dictionary<string, List<Type>>();
		foreach (object[] item in new SerializationTestCases.That_Involve_Constrained_Types())
		{
			string serialized = item[0] as string;
			Type deserializationType = item[1] as Type;
			if (deserializationType.GetName() != "NonEmptyGuids")
			{
				continue;
			}
			...
			try
			{
				...
				try
				{
					deserialized = JsonSerializer.Deserialize(serialized, deserializationType, OptionsInvolvingConstrainedTypesHandling);
				}
				catch (Exception ex)
				{
					var type = deserializationType.GetName();
					Assert.Fail($"[DE-SERIALIZATION] Type {type} ({serialized}):{Environment.NewLine}{ex}");
				}
				...
				try
				{
					reserialized = JsonSerializer.Serialize(deserialized, deserializationType, OptionsInvolvingConstrainedTypesHandling);
				}
				catch (Exception ex)
				{
					var type = deserializationType.GetName();
					Assert.Fail($"[RE-SERIALIZATION] Type {type} ({serialized}):{Environment.NewLine}{ex}");
				}

				if (reserialized == null) { return; }
				Assert.That(reserialized, Is.EqualTo(serialized), $"Type {deserializationType.GetName()} ({serialized})");
			}
			catch
			{
				Assert.Fail($"Serialization test failed for {deserializationType.GetName()}");
			}
		}
		//if (serialized != "[[[[\"00000000-0000-0000-0000-000000000001\"]],[[\"00000000-0000-0000-0000-000000000001\"]]],[[[\"00000000-0000-0000-0000-000000000001\"]],[[\"00000000-0000-0000-0000-000000000001\"]]]]")
		//{
		//	var v = 0;
		//	return;
		//}
	...
	//			{
	//				var obj = NonEmptyGuid.ApplyConstraintsTo("63de52ef-2944-40a5-8183-b656eaf2574f");
