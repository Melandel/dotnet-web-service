...
static class Make
{
	...
	static object MakeArchetype(ConstrainedTypeInfo constrainedTypeInfo, ArchetypeTitle title)
	{
		if (!ArchetypesByConstrainedTypeInfo.ContainsKey(constrainedTypeInfo))
		{
			...
			var yetAnotherValue = constrainedTypeInfo.InvokeStaticFactoryMethod(constrainedTypeInfo.ValidValueExamples[2 % constrainedTypeInfo.ValidValueExamples.Count]);
