...
public class ConstrainedTypeModelBinder : IModelBinder
{
	...
	async Task<string> GetValueAsJsonFrom(ModelBindingContext bindingContext)
	{
		...
		else
		{
			...
			return value.AsValidJsonValue(ConstrainedTypeInfos.TryGet(bindingContext.ModelType, out var constrainedTypeInfo) ? constrainedTypeInfo.RootType : bindingContext.ModelType);
		}
	}
}
