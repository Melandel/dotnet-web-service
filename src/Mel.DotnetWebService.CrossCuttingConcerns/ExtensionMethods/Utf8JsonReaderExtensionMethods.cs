...
{
	static readonly Lazy<Type[]> Utf8JsonReaderNativelySupportedTypes = new Lazy<Type[]>(() => typeof(Utf8JsonReader).GetMethods().Where(m => m.Name.StartsWith("Get")).Select(m => m.ReturnType).Distinct().ToArray());
