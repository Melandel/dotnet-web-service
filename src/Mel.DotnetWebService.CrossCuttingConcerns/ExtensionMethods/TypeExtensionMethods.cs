...
public static class TypeExtensionMethods
{
	...
	public static bool HasFieldOrPropertieWithConstrainedType(this Type type, bool browseRecursively = false)
	{
		...
		// 👇 Prevent infinite recursive loop from System.Collections objects (dictionaries for example)
		...
		.Where(f => f.FieldType != type && !f.Name.Contains("k__BackingField")).ToArray();
		...
		.Where(fieldType => fieldType.Namespace is not null && (fieldType.FullName!.StartsWith("System.Collections.") || !fieldType.Namespace.StartsWith("System")))
		.ToArray();
		...
		.Where(prop => prop.PropertyType != type).ToArray();
		foreach (var p in ps)
		{
		...
		.Where(propertyType => propertyType.Namespace is not null && (propertyType.FullName!.StartsWith("System.Collections.") || !propertyType.Namespace.StartsWith("System")))
		.ToArray();
	...
	=> type.GetInterfaces()
		...
		.Any(itf => itf.GetGenericTypeDefinition() == typeof(IDictionary<,>) && itf.GetGenericArguments().Any(argType => argTypePredicate.Invoke(argType) == true));
}
