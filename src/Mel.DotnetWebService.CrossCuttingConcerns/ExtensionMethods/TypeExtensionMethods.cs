...
public static class TypeExtensionMethods
{
	...
	internal static bool DefinesInstanceConstructorsAndHasNoSetterProperty(this Type type, out ConstructorInfo[] constructors)
	{
		var setterProperties = type.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Where(prop => !prop.Name.Contains("k__BackingField") && !prop.CanWrite).ToArray();
		if (setterProperties.Where(prop => prop.PropertyType != typeof(Type)).Any())
		{
	...
	public static bool IsASingleCollectionEncapsulation(this Type type, out Type? encapsulatedCollectionItemType)
	{
		if (type.HasSingleConstructorAndThatConstructorHasSingleParameterAssignableFrom(BindingFlags.Public, out encapsulatedCollectionItemType))
		{
			return true;
		}

		if(type.HasSingleConstructorAndThatConstructorHasSingleParameterAssignableFrom(BindingFlags.NonPublic, out encapsulatedCollectionItemType) && type.HasStaticFactoryMethodsTakingSingleParameterAssignableFrom(encapsulatedCollectionItemType!.MakeArrayType()/*typeof(IEnumerable<>).MakeGenericType(encapsulatedCollectionItemType)*/, BindingFlags.Public, out _))
		{
			return true;
		}

		encapsulatedCollectionItemType = null;
		return false;
	}

	static bool HasSingleConstructorAndThatConstructorHasSingleParameterAssignableFrom(this Type type, BindingFlags accessModifier, out Type? collectionParameterItemType)
	{
		var constructors = type.GetConstructors(accessModifier | BindingFlags.Instance);
		if (constructors.Length != 1)
		{
			collectionParameterItemType = null;
			return false;
		}
		...
		{
			collectionParameterItemType = argumentTypes.First();
			return true;
		}
	...
	public static bool HasStaticFactoryMethodsTakingSingleParameterAssignableFrom(this Type type, Type parameterType, BindingFlags accessModifiers, out MethodInfo[] factoryMethods)
	{
		var allPublicOrInternalStaticFactoryMethods = type
			...
			{
				[var p] when p.ParameterType.IsAssignableFrom(parameterType) => true,
				_ => false
			});
	...
	public static object CreateInstanceUsingConstructorOrFactoryMethod(this Type type, object instanciationParameterValue, BindingFlags accessModifiers)
	{
		return instanciationParameterValue.GetType() switch
		{
			...
			Type paramType when type.HasStaticFactoryMethodsTakingSingleParameterAssignableFrom(paramType, accessModifiers, out var factoryMethods) => type.CreateInstanceUsingAFactoryMethod(instanciationParameterValue, factoryMethods),
