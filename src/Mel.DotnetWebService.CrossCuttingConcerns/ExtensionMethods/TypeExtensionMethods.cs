...
public static class TypeExtensionMethods
{
	...
	public static string GetName(this Type type)
	{
		try
		{
			...
			return type.IsArray
				? $"{type.GetElementType()!.GetName()}[]"
				: GetCSharpTypeName(type);
		}
		catch
		{
	...
	public static bool IsAFirstClassCollectionWithIConstrainedCollection(this Type type, out Type collectionTypeUsedAsInstanciationParameter)
	{
		if (type.IsGenericType)
		{
	...
	public static bool IsACollectionfKeyValuePairs(this Type type, out Type keyValuePairType, out Type keyType, out Type valueType)
	{
		...
		keyValuePairType = argType;
		var kvpTypes = keyValuePairType.GetGenericArguments();
		keyType = kvpTypes[0];
		valueType = kvpTypes[1];
		return true;
	}
	public static bool IsAFirstClassCollectionWithIConstrainedCollectionOfKeyValuePairs(this Type type, out Type collectionTypeUsedAsInstanciationParameter)
	{
		if (type.IsGenericType)
		{
			collectionTypeUsedAsInstanciationParameter = null!;
	...
	{
		if (type.ImplementsGenericInterface(typeof(IConstrainedValue<,>), out _))
		{
