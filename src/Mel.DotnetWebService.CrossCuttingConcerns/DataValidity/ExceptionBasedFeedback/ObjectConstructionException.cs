...
public partial class ObjectConstructionException : Exception
{
		...
		IEnumerable<Type> methodGenericTypeParameters)
	{
		...
		{
			if (!String.IsNullOrEmpty(externalClassThatImplementsTheInstantiation.Namespace) && !string.Equals(externalClassThatImplementsTheInstantiation.Namespace, objectUnderConstructionType.Namespace))
			{
