...
public partial class ObjectConstructionException : Exception
{
		...
		string callerMethodName)
	{
		return new(
			...
			BuildMessageAboutSpecificMemberConstructionFailure(
				...
				externalClassThatImplementsTheInstantiationState: null,
	...
	ObjectConstructionException AddInformationToObjectConstructionFailureContext<TObjectUnderConstruction, TOtherClass>(
		TOtherClass otherClassState,
		IEnumerable<object?> constructionMethodParameters,
		IEnumerable<Type> methodGenericTypeParameters,
		string callerMethodName)
	...
	static string BuildMessageAboutInstanceConstructionFailure(
		...
		string? ruleThatInvalidatesTheInstanceConstruction,
		string callerMethodName,
		object? externalClassThatImplementsTheInstantiationState,
		...
		IEnumerable<Type> methodGenericTypeParameters)
	{
		...
		var methodOwningType = externalClassThatImplementsTheInstantiation ?? objectUnderConstructionType;
		sb.Append($"via ");
		sb.Append($"{BuildCallerMethodSignatureApproximation(methodOwningType, callerMethodName, constructionMethodParameters, methodGenericTypeParameters)} ");
		...
		if (methodOwningType?.Namespace != null)
		{
			sb.Append($"[{methodOwningType.Namespace}]");
		}

		if (externalClassThatImplementsTheInstantiationState != null)
		{
			sb.Append(' ');
			sb.Append(externalClassThatImplementsTheInstantiationState.RenderObjectState());
		}
		return sb.ToString();
	}
