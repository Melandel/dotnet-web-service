...
static class TypeCategoryResolver
{
		...
		if (!categoriesByType.ContainsKey(type))
		{
			categoriesByType.Add(
				...
				type switch
				{
					...
					var t when IsConstrainedGenericCollectionOfKeyValuePairsType(t) => t is { IsGenericType: true }
						? TypeCategory.ConstrainedGenericCollectionOfKeyValuePairsType
						: TypeCategory.ConstrainedNonGenericCollectionOfKeyValuePairsType,
	...
	static bool IsConstrainedGenericCollectionOfKeyValuePairsType(Type t)
	{
		return t.ImplementsGenericInterface(typeof(IConstrainedCollectionOfKeyValuePairs<,,>), out _);
	}
