...
class DictionaryReadingOperation : ConstrainedTypeConverterReadingOperation
{
	...
	public override object? Execute(ref Utf8JsonReader reader, Type targetType, JsonSerializerOptions options, JsonSerializerOptions preComputedOptionsWithoutConstrainedTypeConverter)
	{
		...
		while (reader.TokenType != JsonTokenType.EndObject)
		{
			...
			reader.Read();
			var key = ConstrainedTypeConverterReadingOperation.For(keyType).Execute(ref propertyNameReader, keyType, options, preComputedOptionsWithoutConstrainedTypeConverter);

			var value = ConstrainedTypeConverterReadingOperation.For(valueType).Execute(ref reader, valueType, options, preComputedOptionsWithoutConstrainedTypeConverter);
