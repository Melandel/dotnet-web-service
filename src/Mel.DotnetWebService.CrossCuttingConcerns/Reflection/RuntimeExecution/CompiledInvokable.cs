...
public class CompiledInvokable
{
	...
	public static CompiledInvokable FromMethodInfo(MethodInfo methodInfo)
	=> FromExpression(LambdaExpressionBuilder.CreateWithParameterTypes(methodInfo.GetParameters().Select(p => p.ParameterType)).AddCallTo(methodInfo).Build());
